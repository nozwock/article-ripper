...
for i in tqdm(range(START_CH, LAST_CH + 1), "processing...🐧️"):
    ...
    for j in soup.find_all("p")[-10:]:
        if re.search(r"(previous|next)\schapter", j.text, flags=re.IGNORECASE):
            j.extract()
    # h3 to h2
    if soup.find("h3") is None:
        ...
        ep_num = re.findall(r"\d+", heading)[0]
        if ep_num not in ep_history:
            ...
            ep_history.append(ep_num)

    doc_md = html_to_md(soup)
    ep_dir = DEST_DIR.joinpath(f"{ep_num}-episode")
    ep_dir.mkdir(parents=True, exist_ok=True)

    with open(ep_dir.joinpath(f"{ep_num}-ch{i}.md"), "w") as f:
        f.write(doc_md)
